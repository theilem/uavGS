...
private:
    template <typename Type>
    WidgetPopulator&
    populateParameter(Type& val);

    template <typename Type>
    WidgetPopulator&
    populateNamedValue(const Type& val);
...
WidgetPopulator::operator&(Type& val)
{
    if constexpr (is_parameter<Type>::value)
        return populateParameter(val);
    else if constexpr (is_named_value<Type>::value)
        return populateNamedValue(val);
    else
    {
...
template <typename Type>
WidgetPopulator&
WidgetPopulator::operator&(const Type& val)
{
    if constexpr (is_named_value<Type>::value)
        return populateNamedValue(val);
    else
    {
        static_assert(is_parameter<Type>::value || is_named_value<Type>::value,
...
template <typename Type>
WidgetPopulator&
WidgetPopulator::populateParameter(Type& val)
{
...
template <typename Type>
WidgetPopulator&
WidgetPopulator::populateNamedValue(const Type& val)
{
    static_assert(is_named_value<Type>::value, "Can only handle parameters");
    using ValueType = typename Type::ValueType;
